...
def linear_svc_count(x_train, y, test, random_state, ):
    vectorizer = CountVectorizer(ngram_range=(1, 3), )
    counts = vectorizer.fit_transform(x_train.values, )
...
def linear_svc_tf_idf(x_train, y, test, random_state, ):
    ...
    return 'linear SVC/tf-idf', LinearSVC(C=1.0, class_weight=None, fit_intercept=fit_intercept, dual=True,
                                          intercept_scaling=1,
                                          loss=loss, max_iter=20000, penalty=penalty, random_state=random_state,
                                          tol=0.0001, verbose=0, ).fit(X=counts, y=y.values, ).predict(
        X=vectorizer.transform(test, ))
...
def random_forest_count(x_train, y, test, random_state, ):
    vectorizer = CountVectorizer(ngram_range=(1, 3), )
    counts = vectorizer.fit_transform(x_train.values, )
    return 'random forest/count', RandomForestClassifier(n_estimators=100, random_state=random_state,
                                                         ).fit(X=counts, y=y.values,
                                                               ).predict(X=vectorizer.transform(test, ), )
...
def random_forest_tf_idf(x_train, y, test, random_state, ):
    vectorizer = TfidfVectorizer(ngram_range=(1, 3), )
    counts = vectorizer.fit_transform(x_train.values, )
    return 'random forest/tf-idf', RandomForestClassifier(n_estimators=30, random_state=random_state,
                                                          ).fit(X=counts, y=y.values,
                                                                ).predict(X=vectorizer.transform(test, ), )
